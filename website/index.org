...
<img src="keeper7keys.jpg" style="float:right;position:absolute;right:10px;top:10px;" width="100em" alt="klutshnik Logo: Keeper of the Seven Keys" />
<ul >
...
Yawn. Even PGP or Age can do that. Yes, but can they rotate keys without decrypting terabytes of data? Can they shard your key across multiple servers so it *never* exists in one place? Can they guarantee that if your key is stolen today, the thief can't decrypt backups from yesterday (Forward Secrecy) or tomorrow (Post-Compromise Security)?
...
*** Messaging (Data-in-Transit)
Messaging apps like Signal or WhatsApp provide **Forward Secrecy (FS)** and **Post-Compromise Security (PCS)**. They generate new ephemeral keys for every message. If a key is stolen, the attacker can strip that one message, but they can't decrypt previous messages (forward secrecy) and future messages (Post-Compromise Security).

*** Storage (Data-at-Rest)
Encryption tools like PGP or Age use a static model:
1.  **DEK (Data Encryption Key):** A random symmetric key used to encrypt the actual file.
2.  **KEK (Key Encryption Key):** Your long-term private key (or password) used to encrypt the DEK.

The generic flow is: $EncryptedFile = Encrypt(Data, DEK) + Encrypt(DEK, KEK)$.

The main issue with this model is that the KEK is static. If someone steals your PGP private key (KEK), they can decrypt every file you ever saved (no Forward Secrecy) and every file you save in the future (no Post-Compromise Security) until you manually re-encrypt everything.

** The Solution: Oblivious Key Rotation
In Klutshnik, the KEK is shared among all the KMS and only used in an Oblivious Pseudo-random Function
...
3.  **Initialize:**
    Run ~klutshnik init~ to generate your keys.

...
properties (PCS & FS) are achieved by generating new ephemeral keys for every message. If a key is stolen, the attacker can decrypt that one message, but they can't decrypt previous messages (FS) and future messages (PCS).

*** Data-at-Rest
But how do you actually do that with data that you send to your future
...
The generic flow is: $EncryptedFile = Encrypt(DEK, KEK) + Encrypt(Data, DEK)$.
...
Klutshnik allows you to rotate your KEK without ever downloading or decrypting the data. It provides forward secrecy since old backups cannot be decrypted with today's keys. It also provides post-compromise security since if your device is hacked, you run a meaningful key rotation and the attacker's stolen key becomes useless for future data access.
...
not traversing the internet through servers harvested by adversaries. 
...
1.  **Install the ~klutshnik~ Client:**
...
This project was funded through [[https://nlnet.nl/entrust][NGI0 Entrust]], a fund established by

...
communicates with a minimum number of these servers (the *threshold*)
to collectively derive a key.

Security is maintained through a *blinded protocol*. When you perform
an operation, the client sends *blinded input*, a mathematically masked
...
The Klutshnik
protocol only makes sense if the KM server is located

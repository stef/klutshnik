...
- **Espressif:** Seeedstudio [[https://wiki.seeedstudio.com/xiao_esp32s3_getting_started/][xiao_esp32s3]] (USB + BLE)
- **Raspberry Pi:** [[https://www.raspberrypi.com/products/raspberry-pi-pico-2/][Pico 2(W)]] (USB)
- **Teensy:** [[https://www.pjrc.com/store/teensy41.html][4.1]] and [[https://www.pjrc.com/store/teensy40.html][4.0]] (USB)

Currently, only the xiao​_esp32s3 supports wireless connections using Bluetooth LE (BLE), the other boards can only communicate using USB connections.
...
they are based on ESP32s3 or RP2350 micro-controllers.
...
You need python tools and the Zephyr SDK.
#+BEGIN_SRC sh
...
If you target a **Teensy** device, you also need the [[https://www.pjrc.com/teensy/loader_cli.html][teensy loader cli version]] and
the ~arm-none-eabi~ cross-compiler toolchain.   
~arm-none-eabi~ cross-compiler toolchain.

For the **Raspberry Pico2** device, you also need the ~arm-none-eabi~
...
First, connect your device via USB (even for the BLE xiao_esp32s3).
...
However, if you use an ESP32s3-based device, you must use the ~esp~ mode:
...
This allows your hardware device to "talk" to the other servers during key rotation updates.
...
For initial provisioning to be completed, you must run the above 2 commands, and have at least 3 entries in the ~authorized_keys~ file. To check completion, run:
...
** 2. (Optional) Monitor Device Logs
...
In a third terminal, run the test suite. This simulates a client performing a full key rotation cycle involving your hardware device. For this, you need to [[TODO: link to client install page][have the

...
[[https://github.com/stef/klutshnik-zephyr][~klutshnik-zephyr~]] is a port of the ~klutshnikd~ server to Zephyr OS
...
Make sure that the
~authorized_keys~ file contains all the other devices you already have that you want to
use.
...
The repository ships with a configured 5-way test setup in the ~test/~ directory. It runs 5 servers including a hardware device.
...
** (Optional) Monitor the device logs

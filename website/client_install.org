...
- *~verbose~*: Controls log chatter. Keep it `false` unless you are debugging.
- *~keystore~*: Path to the local cache for non-sensitive metadata. The default location should be good enough in most deployments.
...
- *~id_salt~*: This random string is used to derive your User ID for each server. With a proper salt, your User ID is different on every server ("blinded ID"). Even if all servers collude and compare logs, they cannot mathematically prove that user A on Server 1 is the same as User B on Server 2.
...
You must add the long-term signing public key to the ~authorized_keys~ file of every server you want to use. Technicalhis key is

...
All cryptographic operations happen locally on your device, ensuring your data never leaves your device in plaintext. The client talks to these servers using a **Verifable Oblivious Pseudo-Random Function (VOPRF)** protocol (based on [[https://datatracker.ietf.org/doc/rfc9497/][RFC 9497]]) in a threshold settin, allowing them to help you unlock your data without ever learning your secrets.
...
support for threshold OPRFs. An OPRF is a cryptographic protocol that allows a 
...
- *~verbose~*: Controls the logging detail. You probably want to keep
  it ~false~ if everything works, and only temporarily enable it to
  debug issues
...
- *~id_salt~*: This random string is used to generate all key IDs for your records. It has no default and you must set this value. If you lose this value, you will lose access to all your keys.
...
- *~bleaddr~*: The Bluetooth LE MAC address of the device. This
  ...
  USB or BLE.
- *~device_pk~*: The public [[http://noiseprotocol.org][Noise protocol]] key of the device.

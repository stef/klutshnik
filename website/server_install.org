...
This is the easiest way to run a server. We provide ready-to-use OS [[https://github.com/stef/klutshnik/releases/latest][images for Raspberry Pi 3 and
...
the root and ~"klutshnik"~ password, which you should change
...
1. You need **Zig v0.15.x**. You can install from the package manager of your distro if it has Zig or from [[https://ziglang.org/download/][ziglang.org]].
...
We recommend running ~klutshnikd~ on port 443.
...
~klutshnikd~ reads configuration files in this specific order, with later
...
- *~ltsigkey~*: The path pointing to the long-term signing private key. You can create one by running ~klutshnikd init~. This will also create a public key and its base64 encoded variant, which should be published to all potential users so that they can use your server in a threshold setup. If running a public server, establish an online presence (static webpage, social media, etc.) to publish your public key
...
- *~authorized_keys~*: A path to a file which stores the public keys of all other clients and KM servers that are allowed to access this server.
...
Ideally, you should run ~klutshnikd~ on a dedicated machine,
...
surface, and you want to run ~klutshnikd~ on a server with other
(probably) HTTPS services. You might also want to run a separate webserver for public
announcements related to your server, like public keys, sample configs, updates and news. We recommend running ~klutshnikd~
...
~klutshnikd~. **DON'T**. Klutshnik as a protocol cannot be TLS-terminated
...
~klutshnikd~ runs on ~127.0.0.1:2523~ and the other stuff on a

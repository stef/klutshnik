...
This is the easiest way to run a server. We provide a ready-to-use OS image based on Alpine Linux, pre-hardened for security.

[[https://github.com/stef/klutshnik/releases/tag/v0.3.0][images for Raspberry Pi 3 and
...
The image generates a self-signed certificate at `/etc/klutshnik/cert.pem`. For local or VPN use, this is fine. In case you prefer proper TLS certificates the image has ~acme.sh~,
...
*** Optional: Enable Tor Hidden Service
...
To allow the server to bind to port 443 without running as ~root~ (dangerous!), use ~setcap~:
...
upstream. Also if you look into plaintext Klutshnik traffic, all you
...
webserver listening on 127.0.0.1:443.

If you want your webserver to have access to IP addresses of

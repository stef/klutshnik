...
Server (KMS) protocol. In this system, the client interacts with a group
...
the KMS servers. This key can be stored on disk and referenced in the `klutshnik.cfg(5)` config file. Alternatively, `klutshnik` can
...
(https://github.com/stef/pwdsphinx) as a secure "storage" for the client's
long-term signing keys (ltsigkey).

### Key Names

Key names are arbitrary strings used to identify your records.
Internally, these names are hashed using the `id_salt` from the
configurations `[client]` section into a unique key identifier.
Given that this `id_salt` is critical for accessing your records,
it is important to use a unique salt and maintain a secure backup of
...
Creates a new key identified by `<keyname>`. `ltsigkey` is the long-term signing private key as described above, and should be provided if the `ltsigkey_path` (see klutshnik.cfg(5)) config option is not set.
...
**SECURITY WARNING:** The update delta token must be kept secret. If
...
The following metadata are refreshed, and each clients should run this operation to refresh their local copy:
...
If your ltsigkey is not in the configuration (see klutshnik.cfg(5)),
it must be provided on standard input.
...
owners client long-term signing and Noise public keys. A new `servers` sub-section, having either the Bluetooth MAC or `usb-cdc0` as a name, is added to `klutshnik.cfg`, the client config file.
...
- **Delta Update Tokens:** You **MUST** keep delta update tokens private.
  Leaking them compromises the system’s forward secrecy and
  post-compromise security.
- **Key Storage:** It is **RECOMMENDED** to store your master signing key
...
- **Backups:** You **SHOULD** back up your configuration, especially the `id_salt`

...
klutshnikd - klutshnik Key Management Server (KMS) daemon
...
Server (KMS) protocol. It provides a secure, sharded environment for
...
You **SHOULD** maintain secure backups of:

- Long-term signing and Noise keys.
- TLS certificates `ssl_cert` and private keys `ssl_key`.
- The internal database containing sharded key material.
- The `record_salt` value used for internal ID hashing.

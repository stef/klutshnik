...
Key Management Server (KMS) daemon.
...
directory must be backed up securely. Default: `/var/lib/klutshnikd`.

### `MAX_KIDS`

The maximum number of concurrent handlers (sessions) allowed.
Adjust this based on expected load and available system resources.
Default: 5.

### VERBOSE

A boolean value that enables detailed logging of server operations to standard
output.
...
The key can be generated by running the `klutshnikd(1)`
...
authenticated key exchange. The key can be generated by running the `klutshnikd(1)`
client with the `init` parameter:
...
keys from other KMS servers. This is checked on create and
